...
const getProductsByCategoryName = (categoryName) => {
	const queryTemplate = `
    SELECT make, model, image_url, description, rent_rate_in_cents, price_in_cents, products.id as id, categories.name as category_name, sub_categories.name as sub_category_name
    FROM categories
